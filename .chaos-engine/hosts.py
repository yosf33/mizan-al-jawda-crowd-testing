...
def parse_codex_marketplace_list(stdout: str) -> dict:
    marketplaces = []
...
def parse_codex_plugin_list(stdout: str) -> dict:
    installed = []
    lines = stdout.splitlines()
    for line in lines:
        line = line.strip()
        if not line or line.startswith("Marketplace") or line.startswith("/") or line.startswith("PLUGIN"):
            continue
        parts = re.split(r'\s{2,}', line)
        if len(parts) >= 3:
            plugin_id = parts[0]
            ...
            is_installed = "installed" in status_str
...
def client_command(
    ...
    if os.path.basename(executable) == "codex":
        arguments = [arg for arg in arguments if arg not in ("--json", "--available")]
    ...
    try:
        ...
        if os.path.basename(executable) == "codex":
            if "marketplace" in arguments:
                return parse_codex_marketplace_list(result.stdout)
            else:
    ...
    *,
    ...
    for client in ("codex", "claude"):
        ...
        try:
            ...
            plugin_states = {name: "absent" for name in plugins}
...
def activation_commands(root: Path, plugin_id: str) -> dict[str, dict[str, list[str]]]:
    return {
        ...
        "claude": {
            "marketplace": ["plugin", "marketplace", "add", str(root)],
