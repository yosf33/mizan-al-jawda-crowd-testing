...
## Local Installation (For this Machine)

If you are installing ChaosEngine from your local copy at `/Users/youssf/chaos-engine-demo` onto a new project, use these steps:

1. **Navigate to the target project** where you want to install ChaosEngine:
   ```bash
   cd "/path/to/your/project"
   ```

2. **Clean up any previous or partial installations** (recommended to avoid conflicts):
   ```bash
   rm -rf .chaos-engine .chaos-engine.backup .chaos-engine-cross-rollback .chaos-engine-dependencies.json .chaos-engine-hosts.json .chaos-engine-runtime-current.json GEMINI.md AGENTS.md CHAOS_PLAYBOOK.md .agents plugins .claude .claude-plugin .codex .gemini .mcp.json mempalace.yaml .chaos-engine-runtime-generations .chaos-engine-runtime-transactions .chaos-engine-runtime.lock .chaos-engine-state .chaos-engine.lock
   ```

3. **Copy the patched files & plugins** from the local source directory (excluding dynamically generated files):
   ```bash
   cp -R "/Users/youssf/chaos-engine-demo/.chaos-engine" .
   cp -R "/Users/youssf/chaos-engine-demo/.agents" .
   cp "/Users/youssf/chaos-engine-demo/GEMINI.md" .
   cp "/Users/youssf/chaos-engine-demo/AGENTS.md" .
   cp "/Users/youssf/chaos-engine-demo/CHAOS_PLAYBOOK.md" .
   cp "/Users/youssf/chaos-engine-demo/CLAUDE.md" .
   cp -R "/Users/youssf/chaos-engine-demo/plugins" .
   rm -f .chaos-engine/manifest.json
   ```

4. **Run the local installer**:
   ```bash
   python3 /Users/youssf/chaos-engine-demo/.chaos-engine/install.py install \
     --project . \
     --source "/Users/youssf/chaos-engine-demo/.chaos-engine" \
     --commit "3a277e066878b6177a1af540af2ae260f38abc02"
   ```

5. **Manually activate the client plugins** (for Codex CLI and Claude Code):
   ```bash
   python3 -c "import sys; sys.path.insert(0, '.chaos-engine'); import hosts; from pathlib import Path; hosts.activate_detected_plugins(Path('.'))"
   ```

6. **Verify health** with the doctor command:
   ```bash
...
During the setup on this machine, we resolved and patched the following installation blockers in `.chaos-engine/hosts.py`:

* **Codex CLI Compatibility (`codex-cli 0.133.0`)**:
  Older versions of `codex-cli` do not support `--json` or `--available` flags on `plugin list` and `plugin marketplace list` commands. We patched `hosts.py` to strip these options and dynamically parse plain-text output tables, preventing execution crashes.
* **Claude Code CLI Compatibility (`Claude Code 2.1.19`)**:
  `claude plugin marketplace add` and `claude plugin marketplace remove` do not accept a `--scope` argument. We patched `hosts.py` to exclude `--scope local` for these marketplace-specific commands.
* **MemPalace Configuration Validation**:
  If an existing `mempalace.yaml` does not contain `exclude_patterns` with list items, the validation checks will fail. Wiping any old or incorrect `mempalace.yaml` before running the installer allows it to auto-generate a valid configuration.
* **Reserved Manifest Collision**:
  The source folder for `--source` must *not* contain `manifest.json` as it is generated dynamically at the target destination. If you see `ValueError: source contains the reserved manifest path`, run `rm -f /Users/youssf/chaos-engine-demo/.chaos-engine/manifest.json` before running `install.py`.
