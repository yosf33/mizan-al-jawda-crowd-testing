...
import { ArrowRight, BriefcaseBusiness, CircleDollarSign, LogOut, ShieldCheck, Smartphone, TestTube2 } from "lucide-react";
...
export default function Onboarding() {
  ...
  if (!user) {
    return <main dir="rtl" className="onboarding-page min-h-screen grid place-items-center bg-[#f8f4e9] p-6"><section className="onboarding-guest-card surface-card p-8 text-center"><ShieldCheck className="mx-auto h-8 w-8 text-[#b8882e]" /><h1 className="mt-4 text-2xl font-bold">ابدأ من حساب موثّق</h1><p className="mt-2 text-[#65727c]">نسجّل الهوية أولاً لحماية المشاريع والمكافآت.</p><Button className="mt-6 w-full bg-[#102a43]" onClick={() => startLogin()}>تسجيل الدخول</Button></section></main>;
  }

  return (
    <main dir="rtl" className="onboarding-page relative min-h-screen overflow-hidden bg-[#f8f4e9] px-5 py-10 text-right md:py-16">
      <div className="golden-grid absolute inset-0 opacity-70" />
      <section className="relative mx-auto grid w-full min-w-0 max-w-5xl overflow-hidden rounded-[2rem] border border-[#e6d9b5] bg-[#fffdf8]/95 shadow-[0_25px_70px_rgba(25,45,65,.10)] md:grid-cols-[.8fr_1.2fr]" style={{ direction: "rtl" }}>
        <aside className="min-w-0 bg-[#102a43] p-8 text-[#fffdf8] md:p-11">
        ...
        <section className="min-w-0 p-7 md:p-11">
          <div className="mb-8 flex items-center justify-between gap-3">
            ...
            <button type="button" className="flex items-center gap-2 text-sm text-[#7d6b3d] hover:text-[#9f3030]" onClick={() => void handleLogout()}><LogOut className="h-4 w-4" />تسجيل الخروج</button>
          </div>
