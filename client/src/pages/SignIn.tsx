...
export default function SignIn() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try { const { error: signInError } = await requireSupabase().auth.signInWithPassword({ email, password }); if (signInError) throw signInError; setLocation("/workspace"); } catch (cause) { setError(localizeAuthError(cause)); } finally { setBusy(false); }
  };
